...
// Pre-compute lowercase current users into a Set for O(1) lookup.
// Reduces time complexity from O(N*M) to O(N+M) and avoids repeated string allocations from toLowerCase().
// Impact: O(1) lookups instead of O(N) array scans per new user.
