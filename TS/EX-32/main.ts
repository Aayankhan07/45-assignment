...
// Convert current_user to a Set of lowercase strings for O(1) lookups.
// This reduces the username availability check from O(N*M) to O(N+M)
// avoiding repetitive `.some()` and `.toLowerCase()` calls for each new user.
