...
// ⚡ Bolt: Replaced O(N*M) nested array.some() with O(N+M) Set.has() lookup for better performance
const current_user_lower_set = new Set(current_user.map(user => user.toLowerCase()));
